...
| `Ranged` | 관리되는 TACZ·PointBlank·SuperbWarfare 원거리 무기만 사용합니다. |
...
| 설정 | 실제 의미 |
| --- | --- |
| `Max Distance` | 관리 원거리 사격의 최대 타겟 거리입니다. 이보다 먼 타겟은 애드온이 무시합니다. |
...
| 위치 | 정책 |
| --- | --- |
| `Gun` | `Gun Spec`은 운용 중인 총기의 원본 피해 동작을 유지합니다. `Custom`은 원본 총기 데이터를 바꾸지 않고 지원되는 DW 피해 오버라이드를 적용합니다. |
...
`Sound Detection`은 관리 총성, 재장전, 블록 소리, 엔티티 소리, 위치만 있는 월드 소리를 하나의 서버 판정 자극 시스템으로 처리합니다. 서바이벌과 크리에이티브 플레이어는 동일하게 처리하고 관전자 타겟은 제외합니다. 반응 방식은 `Off`, `Look Only`, `Move To Source`입니다. 전투가 시작되면 조사는 전투에 양보하고, 위치 고정 스탠스에서는 `Move To Source`도 바라보기만 수행합니다.

`Faction Defense`를 켜면 피해를 받은 DW 관리 NPC가 `Faction Alert Radius` 안의 같은 팩션 관리 아군에게 공격자를 알립니다. 지원 NPC는 공격자를 아직 직접 보지 못해도 도우러 이동할 수 있지만, 발사 직전에는 여전히 최종 시야가 필요합니다. 용병의 소유자 공격 허용 횟수와 우호 타겟 검증도 유지되므로 팩션 방어가 계약·타겟 규칙을 우회하지 않습니다.
...
| `Custom Pose` | `DW Pose Core`로 저장한 포즈 JSON을 사용합니다. |
...
`Gun`은 플레이어 인벤토리의 지원 TACZ·PointBlank·SuperbWarfare 총기 스택을 읽습니다. 선택한 총기를 바로 저장하거나 여러 지원 총기를 랜덤 무기 풀에 넣을 수 있습니다. 풀 굴림은 리스폰, 청크 로드 초기화, 클론 복원, 소울스톤 복원 같은 CustomNPCs init 이벤트에서 실행될 수 있습니다.

TACZ는 항상 스탠스·애니메이션 기준입니다. 선택형 연동은 외부 총기의 아이템 렌더링, 탄환/히트스캔, 사운드, 재장전 단계, 고유 동작을 유지합니다. PointBlank는 1.11.1과 2.1.0 전용 브리지를 따로 사용하고, SuperbWarfare는 검증된 0.8.9 final 빌드에 고정됩니다. 특수 사격·근접 전용 등 지원하지 않는 총기는 안전하게 거부됩니다.
...
`Visual & FX`는 로컬 CustomNPCs 텍스처 경로나 HTTP/HTTPS URL, 각 항목의 가중치, `Steve`/`Alex` 모델, 오버레이, 망토 설정을 스킨 풀로 저장합니다. 텍스처 자체는 수정하지 않습니다. `Armor`는 실제 머리, 몸통, 다리, 발 방어구 슬롯을 사용하는 전체 방어구 세트를 저장합니다. Gun, Melee, Armor 화면의 미리보기는 선택한 모델, 스킨, 방어구와 무기를 함께 표시합니다.

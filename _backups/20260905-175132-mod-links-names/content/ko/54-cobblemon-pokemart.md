...
0.1.4의 검색과 페이지 이동은 상품·교환 제안·보유 포켓몬·경매 매물·판매 목록·정산 항목을 서버에서 필요한 만큼만 가져옵니다. 큰 상점이나 경매장도 전체 목록을 한꺼번에 보내지 않으며, 검색어와 페이지를 바꾸면 해당 결과만 새로 받습니다.

`Refresh`는 현재 검색과 페이지의 최신 목록·재고를 다시 받아오고 `Close`는 서버의 화면 추적을 해제합니다. 네트워크 재전송이나 더블 클릭으로 같은 요청이 들어와도 최근 요청 ID를 기억해 중복 결제와 중복 지급을 막습니다.

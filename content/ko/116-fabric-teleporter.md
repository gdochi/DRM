...
대상은 플레이어의 현재 차원 안에서 좌표와 회전값을 사용합니다. DRM Core 0.1.8에는 목적지 차원 필드가 없으므로 차원 간 텔레포터로 동작하지 않습니다.
...
0.1.8에서는 목적지 명령을 실행할 시점도 고를 수 있습니다. 페이드 시작 전·도중, 실제 이동 직전·직후, 페이드 인 도중·완료 후 중에서 연출에 맞는 시점을 사용하세요.
...
FUNCTION 검색은 현지화된 액션 이름, 대상 ID, 서버 JSON 종류, 에디터 ID, 바인딩 그룹을 함께 검색합니다. 기능을 바꿀 때 좌측 하단에 잠깐 나타나던 중복 로딩 라벨은 0.1.8에서 그리지 않으며, 실제 적용/불러오기 작업의 피드백은 유지됩니다.
...
일반 `image` 컴포넌트도 `teleporter` 레이아웃에서 사용할 수 있습니다. 0.1.8 플레이어 런타임은 화면을 덮던 바닐라 블러를 적용하지 않으므로 검색, 목적지 선택, 이동, 닫기 컨트롤을 정상 조작할 수 있습니다.

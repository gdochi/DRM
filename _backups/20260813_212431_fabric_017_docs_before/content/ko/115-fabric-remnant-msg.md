...
0.1.6의 메시지/정책 선택 창은 서버 JSON 목록을 검색할 수 있습니다. 마커 외형은 `Default`, `Item`, `Block` 중에서 고르고, 아이템/블록은 이름·네임스페이스·ID로 검색합니다. `Scale`은 `0.05`부터 `16.0`까지이며, `Billboard`는 `fixed`, `vertical`, `horizontal`, `center`를 지원합니다. 아이템 외형은 `none`, 1·3인칭 손, `head`, `gui`, `ground`, `fixed` 변환을 선택할 수 있습니다. 이 값은 서버에서 다시 검증되어 마커 SavedData와 표시 엔티티에 저장됩니다.

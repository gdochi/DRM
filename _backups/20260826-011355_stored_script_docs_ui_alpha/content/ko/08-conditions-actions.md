...
| `stored` | `key`, `value`, `op` | 레거시 DRM 대화 런타임의 플레이어 문자열 값을 비교합니다. |
...
`stored`와 `cnpc_stored_data`는 서로 다른 저장소를 읽습니다. `stored`는 이전 버전 호환을 위한 **Legacy DRM Runtime Value**이고 CustomNPCs의 `getStoreddata()`와 연결되지 않습니다. CustomNPCs 스크립트에서 `event.player.getStoreddata().put(...)`처럼 저장한 값을 읽으려면 반드시 `cnpc_stored_data`를 선택하십시오.

`cnpc_stored_data`는 `player`, `context_entity`, `world` Scope와 `exists`, `not_exists`, 문자열·숫자 비교를 지원합니다. 에디터 사용 순서, 실행 위치별 문맥 차이, 스크립트 예제와 실패 규칙은 스크립트 API 섹션의 **CustomNPCs Stored Data 조건 연동** 문서를 참고하십시오.

...
0.2.5의 비전투 포즈 바인딩은 서버가 관리하는 영속 상태입니다. 플레이어가 NPC 추적을 시작할 때 전체 정의를 동기화하고, 전투가 끝난 뒤 현재 전투 포즈를 덮어쓰지 않으면서 다시 적용합니다. `Save & Apply`는 권한 있는 `idle_standing` 바인딩을 갱신하므로 편집기 미리보기와 소환된 NPC가 같은 프로필을 사용합니다.
...
0.2.5의 내장 목록에는 Low Ready, High Ready와 여러 전투·평시·걷기·이동 사격 포즈가 포함됩니다. 내장 포즈를 적용한 뒤 필요한 축과 총기 위치만 조정하고 `Save As`로 사용자 JSON을 만들면 원본 내장 값은 변경되지 않습니다.

...
## `stored`와 `cnpc_stored_data`의 차이

두 타입은 이름이 비슷하지만 저장소가 완전히 다릅니다.

| 타입 | 에디터 표시 | 읽는 저장소 | CustomNPCs `getStoreddata()`와 연결 |
| --- | --- | --- | --- |
| `stored` | Legacy DRM Runtime Value | DRM 대화 런타임의 플레이어 문자열 값 | 아니요 |
| `cnpc_stored_data` | CustomNPCs Stored Data | CustomNPCs의 플레이어·엔티티·월드 Stored Data | 예 |

CustomNPCs 스크립트에서 `event.player.getStoreddata().put(...)`로 기록했다면 반드시 `cnpc_stored_data`를 선택해야 합니다. 기존 `stored` 조건을 이름만 보고 선택하면 값을 찾지 못합니다. 두 저장소 사이에 자동 마이그레이션이나 동기화는 없습니다.
...
2. Type이 `Legacy DRM Runtime Value`가 아니라 `CustomNPCs Stored Data`인지 확인합니다.

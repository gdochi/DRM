...
order: 500
description: DRM Cobblemon Editor 0.1.3의 기능 범위와 제작 도구입니다.
...
`DRM Cobblemon Editor`는 Fabric 1.21.1용 DRM 애드온입니다. CustomNPCs NPC에 Cobblemon 전투, 전투 전 연출, PokéMart, Nurse Joy, 스타터 선택 기능을 적용하고, 재사용할 NPC 클론과 스포너 소스까지 같은 제작 흐름에서 관리합니다.
...
:::note 이름과 내부 ID
표시 이름은 `DRM Cobblemon Editor`이지만 기존 데이터 호환성을 위해 모드 ID와 리소스 네임스페이스는 `cobble_npc`입니다. `cobble_npc:*` ID를 임의로 바꾸지 마세요.
:::

...
0.2.5에서는 차량 편집기의 `승무원` 탭에서도 NPC 클론을 사용할 수 있습니다. 상면/측면 좌석 지도에서 빈 좌석을 선택하고 `NPC 태우기`를 누른 뒤 서버 NPC 클론 또는 플레이어 인벤토리의 NPC 소울스톤을 고릅니다. 서버는 원본을 정리해 새 NPC를 만들고 선택한 정확한 좌석에 탑승시키며, 적대 팩션·변경된 좌석·잘못된 소울스톤은 거부합니다.

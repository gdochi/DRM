...
> Category: Dialogue / 다이얼로그

This category covers the Dialogue Editor workflow for Dochi Editor. It is the first major content branch under the main DRM wiki category.

이 페이지는 다이얼로그 카테고리의 기준 문서입니다. 한국어와 영어 지원은 `wiki/i18n/`의 문자열과 향후 번역 페이지 확장으로 이어갈 수 있습니다.

[Back to Home / 홈으로 돌아가기](Home.md)
...
This category page is the canonical English source for the Dialogue Editor manual. Localized strings are kept in `wiki/i18n/dialogue-editor-wiki.en_us.json` and `wiki/i18n/dialogue-editor-wiki.ko_kr.json`.
...
![Dialogue Editor overview](wiki/assets/dialogue-editor/overview.svg)
...
![Dialogue Editor workflow](wiki/assets/dialogue-editor/workflow.svg)
...
| Korean and English text drift apart | One locale file was updated without the other. | Keep both locale files in sync. |

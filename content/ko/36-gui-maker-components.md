...
이 문서에서 설명하는 컴포넌트는 GUI Maker 에디터에서 배치하는 화면 부품입니다. 플레이어가 보는 런타임 화면에서는 이 부품 안에 Dialogue Editor의 대사와 선택지, NPC Shop의 상품과 가격, Currency Editor의 잔액 같은 실제 데이터가 들어옵니다.

...
| DRM Core | `dochi_rpg_maker` 0.1.3 | 클라이언트와 서버 양쪽 필요 |
| Java | 17 | Forge 1.20.1 실행 환경과 동일하게 맞춥니다. |
| CustomNPCs | 선택 의존성 | CustomNPCs NPC 대화·상점·NPC 편집 흐름에는 필요 |
...
4. 기본 파일이 설치된 뒤 `dialogue_sets`, `gui`, `npc_shops`, `currency`, `hud`, `remnant_msg` 폴더를 확인합니다.

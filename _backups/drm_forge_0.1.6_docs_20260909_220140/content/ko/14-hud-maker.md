...
**F3**를 켜도 커스텀 HUD와 바닐라 바 대체 설정을 유지합니다. 예를 들어 체력바를 커스텀 바로 바꿨다면, 디버그 화면을 열어도 바닐라 하트로 돌아가지 않습니다.

화폐 HUD에서 **Inventory** 표시를 끄면 서바이벌과 크리에이티브 인벤토리 모두에서 숨겨집니다. **Always**나 금액 변경 알림을 켠 경우에도 같은 기준이 적용됩니다.

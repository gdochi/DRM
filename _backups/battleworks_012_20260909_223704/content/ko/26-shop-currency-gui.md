...
아래 컴포넌트는 GUI Maker에서 `npc_shop` 타입 GUI를 만들 때 배치하는 화면 요소입니다. 플레이어가 보는 인게임 상점 런타임은 이 GUI 배치를 사용해 NPC Shop 문서의 상품, 가격, 재고를 표시합니다.
...
즉, NPC Shop 에디터에서 `items`와 `sellItems`를 고치면 무엇을 사고팔 수 있는지가 바뀝니다. GUI Maker에서 `item_slot`, `shop_action_button`, `currency_display` 위치를 고치면 플레이어가 런타임 상점 화면에서 그것을 어디에 어떤 모양으로 보는지가 바뀝니다.

...
slug: quick-start
order: 20
description: Fabric 1.21.1용 DRM Core 0.1.6을 처음 열고 제작을 시작하는 순서입니다.
product: core-fabric
category: 시작하기
...
2. 서버에서 사용할 경우 서버와 접속 클라이언트 양쪽에 같은 DRM 0.1.6 Fabric JAR을 넣습니다.
3. CustomNPCs NPC에 대화나 상점을 연결할 경우 CustomNPCs도 같은 환경에 설치합니다. DRM 자체의 파일/HUD/GUI 편집만 사용할 때는 로더 수준에서 필수 의존성은 아닙니다.
...
0.1.6의 선택 UI는 검색과 `Built-in` / `Add-on` 분류를 지원합니다. 마지막으로 열었던 에디터와 JSON 소스도 세션 동안 기억하므로, 에디터 선택 화면으로 돌아왔다가 다시 열면 직전 작업 흐름을 이어가기 쉽습니다.

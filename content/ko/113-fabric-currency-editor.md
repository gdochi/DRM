...
Fabric 0.1.6은 플레이어가 사망 후 새 엔티티로 교체될 때 DRM 지갑과 호환 은행 잔액 키를 복사합니다. `KEEP`은 잔액을 유지하고, `LOSE`는 손실 규칙을 적용한 결과를 이어받습니다. 아이템 자동 변환과 사망 손실이 실제로 발생하면 플레이어 채팅에 안내가 표시됩니다. 일반 픽업은 방금 주운 스택만 변환하며 인벤토리의 다른 화폐 스택을 함께 훑지 않습니다.

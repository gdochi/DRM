...
## Migrating the Legacy `stored` Type

The old DRM `stored` condition has been removed. The condition list and new JSON use only `cnpc_stored_data`.

If an existing dialogue, quest, teleporter, or Remnant Message JSON still contains `"type": "stored"`, DRM reads it as a legacy alias of `cnpc_stored_data`. Saving that file through the condition editor canonicalizes `type` to `cnpc_stored_data`. DRM no longer reads a separate legacy runtime-value store.

Older JSON may not contain Scope or Value Type. In that case the defaults are `scope: "player"` and `valueType: "auto"`; the existing `key`, `op`, and `value` remain in use. Select `CustomNPCs Stored Data` for every new condition.
...
2. Confirm that Type is `CustomNPCs Stored Data`. Re-save old `stored` JSON to canonicalize the type.

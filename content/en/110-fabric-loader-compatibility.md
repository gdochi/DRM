...
Every page in this product targets `DRM Core 0.1.6`, `Fabric 1.21.1`, and Java 21. The separate `DRM Forge 1.20.1` product remains the documentation for the Forge build.
...
| Example JAR | `dochi_rpg_maker-0.1.4-1.20.1-forge.jar` | `dochi_rpg_maker-0.1.6-fabric-1.21.1.jar` |
...
## Fabric 0.1.6 Features To Check
...
## Current 0.1.6 Boundary

The later Forge 0.1.4 option `choiceHideUntilTypingComplete` is not present in the current Fabric 0.1.6 source. Fabric has an FTB Quest picker, but do not assume it includes every later Forge picker refinement such as initial search focus and all distinct empty-state messages.

Fabric 0.1.6 has passed a clean build, dedicated-server/client boot, 31 focused regression checks, parsing of 47 source JSON files, and equal key sets across 11 bundled locales. Interactive GUI, multiplayer, live FTB Quests, and reconnect/restart persistence still need acceptance testing for the actual server modpack.
...
5. Test dialogue Load/Save As, shop buy/sell, currency death rules, HUD, and Remnant restart persistence.

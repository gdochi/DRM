...
order: 90
description: Currency Editor에서 디지털·실물 화폐, 아이템 변환, 사망 규칙과 HUD 표시를 설정하는 방법입니다.
product: core-fabric
category: 커런시 에디터
...
0.1.8부터 화폐 정의의 `physicalItem`을 켜면 인벤토리에서 주고받을 수 있는 실물 화폐를 만들 수 있습니다.

| 설정 | 의미 |
| --- | --- |
| 이름 | 아이템에 표시할 이름 또는 번역 키입니다. |
| ID | 같은 기본 아이템 안에서 화폐 종류를 구분하는 고유 ID입니다. |
| 개당 가치 | 아이템 한 개가 나타내는 화폐 금액입니다. |

상점은 설정에 따라 실물 화폐를 결제로 받거나 판매 대금으로 지급할 수 있습니다. 실물 ID는 다른 화폐와 겹치지 않게 정하세요.
...
Fabric 0.1.8은 플레이어가 사망 후 새 엔티티로 교체될 때 DRM 지갑과 호환 은행 잔액 키를 복사합니다. `KEEP`은 잔액을 유지하고, `LOSE`는 손실 규칙을 적용한 결과를 이어받습니다. 아이템 자동 변환과 사망 손실이 실제로 발생하면 플레이어 채팅에 안내가 표시됩니다. 일반 픽업은 방금 주운 스택만 변환하며 인벤토리의 다른 화폐 스택을 함께 훑지 않습니다.
